...
// --- PROPRIETARY PROTECTION & CYBERSECURITY SYSTEM ---
(function () {
  ...
  const check = () => {
    ...
    if (widthThreshold || heightThreshold) {
      if (!devtoolsOpen) {
        console.clear();
        console.log("%c[SECURITY WARNING]", "color: red; font-size: 24px; font-weight: bold;");
        console.log("%cNexaDesk Spatial Workspace Initialized", "color: orange; font-size: 14px;");
        devtoolsOpen = true;
      }
    } else {
      devtoolsOpen = false;
        ...
        content = `
          <div class="file-explorer-container" style="display:flex; flex-direction:column; height:100%; width:100%;">
            <div class="file-explorer-header" style="display:flex; align-items:center; gap:8px; padding:6px 12px; background:rgba(0,0,0,0.2); border-bottom:1px solid var(--glass-border);">
              ...
              <span id="file-current-path" style="font-family:var(--font-mono); font-size:11px; color:rgba(255,255,255,0.7);">/</span>
            </div>
        ...
        content = `
          <div class="ide-container">
            <div class="ide-toolbar">
              <div class="ide-tabs">
                <div class="ide-tab active-tab" data-file="page.tsx">page.tsx</div>
                <div class="ide-tab" data-file="globals.css">globals.css</div>
                <div class="ide-tab" data-file="server.js">server.js</div>
                <div class="ide-tab" data-file="api/route.ts">api/route.ts</div>
                <div class="ide-tab ai-tab" data-file="assistant">Code Assistant</div>
              </div>
            ...
            <div class="ide-workspace">
              ...
              <div class="ide-ai-wrapper" id="ide-ai-wrapper">
                ...
                <div class="ai-input-box">
                  <input type="text" class="ai-prompt-input" id="ai-prompt-input" placeholder="Describe what you want to build..."/>
  ...
  <main class="min-h-screen bg-gray-950 text-white font-sans">
    ...
    document.querySelectorAll('.ide-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        document.querySelectorAll('.ide-tab').forEach(t => t.classList.remove('active-tab'));
        ...
        if (activeFile === 'assistant') {
          ...
          this.showToast("CODE ASSISTANT ACTIVE");
        } else {
          if (aiWrapper) aiWrapper.classList.remove('active-ai');
          editorArea.style.display = 'flex';
          codeInput.value = files[activeFile] || "";
          updateLineNumbers();
          this.showToast(`SWITCHED FILE: ${activeFile}`);
        }
      });
    });
    ...
    // Code Assistant Prompt Generator (with Ollama, Web-LLM & Custom API support)
      ...
      try {
        if (provider === 'heuristics') {
          ...
          const res = await fetch(endpointUrl, {
            ...
            body: JSON.stringify({
              model: modelName,
              prompt: `User request: ${promptText}\n\nYou are a coding assistant. Return only functional code or brief instructions. Format code in markdown blocks (e.g. \`\`\`html, \`\`\`css, \`\`\`js).`,
              stream: false
            })
          });
          ...
          const res = await fetch(`https://api.openai.com/v1/chat/completions`, {
            ...
            body: JSON.stringify({
              model: model,
              messages: [
                { role: 'system', content: 'You are a helpful coding assistant. Return code in markdown blocks.' },
...
\`\`\``;
    ...
    // Restore active tab styling from saved session
    if (activeFile !== 'page.tsx') {
      document.querySelectorAll('.ide-tab').forEach(t => {
        t.classList.remove('active-tab');
        if (t.getAttribute('data-file') === activeFile) t.classList.add('active-tab');
      });
      if (activeFile === 'assistant') {
        editorArea.style.display = 'none';
        if (aiWrapper) aiWrapper.classList.add('active-ai');
